...
class GoogleReverseImageSearch:
    ...
    def response(
        self, query: str, image_url: str, max_results: int = 10, delay: int = 1
    ) -> SearchResults:
        ...
        if len(all_results) == 0:
            logging.warning(
                f"No results were found for the given query: [{query}], and/or image URL: [{image_url}]."
            )
    ...
    def _validate_input(self, query: str, image_url: str):
        ...
        if not image_url:
            raise ValueError(
                "Image URL not found. Please enter an image URL and try again."
            )
        if not self._validate_image_url(image_url):
            raise ValueError(
                "Invalid image URL. Please enter a valid image URL and try again."
            )
    ...
    def _extract_result_data(self, result) -> Dict:
        link = (
            result.find("a", href=True)["href"] if result.find("a", href=True) else None
        )
